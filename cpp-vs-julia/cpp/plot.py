...
# CSVファイルのパス
output_dir = "./run/"
num_time_steps = 1000  # 必要に応じて調整してください
time_step = 0.1  # C++コード
...
def create_animation(output_dir, num_time_steps):
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    cax = ax.imshow(read_csv(os.path.join(output_dir, "output_0.csv")), cmap='inferno', interpolation='nearest', origin='lower', vmin=0, vmax=10, aspect='auto')
    fig.colorbar(cax)
...
# メイン関数
def main():
    create_animation(output_dir, num_time_steps)
